...
class VirtualNodeRpcTest {
    ...
    @Test
    @Order(81)
    fun `can run a flow after force uploading the same CPI`() {
        cluster {
            endpoint(CLUSTER_URI, USERNAME, PASSWORD)
    ...
    @Test
    @Order(82)
    fun `can run a flow on original CPI to persist a dog`() {
        cluster {
            endpoint(CLUSTER_URI, USERNAME, PASSWORD)
    ...
    @Test
    @Order(90)
    fun `can force upload the same version of a CPI with a new set of CPKs`() {
        cluster {
            endpoint(CLUSTER_URI, USERNAME, PASSWORD)
