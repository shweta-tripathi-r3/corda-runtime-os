java    -Dlog4j.configurationFile=log4j2-console.xml \
        -jar ./db-worker/build/bin/corda-db-worker-5.0.0.0-SNAPSHOT.jar \
...
java    -Dlog4j.configurationFile=log4j2-console.xml \
        -jar ./flow-worker/build/bin/corda-flow-worker-5.0.0.0-SNAPSHOT.jar \
	--messagingParams kafka.common.bootstrap.servers=127.0.0.1:9092 \
	--workspace-dir /tmp/corda \
...
java    -Dlog4j.configurationFile=log4j2-console.xml \
        -jar ./member-worker/build/bin/corda-member-worker-5.0.0.0-SNAPSHOT.jar \
	--messagingParams kafka.common.bootstrap.servers=127.0.0.1:9092 \
...
java    -Dlog4j.configurationFile=log4j2-console.xml \
        -jar ./rpc-worker/build/bin/corda-rpc-worker-5.0.0.0-SNAPSHOT.jar \
	--messagingParams kafka.common.bootstrap.servers=127.0.0.1:9092 \
        --healthMonitorPort 7004 \
        | tee -a rpc-worker-$(date "+%Y-%m-%dT%H:%M:%S").log &

java    -Dlog4j.configurationFile=log4j2-console.xml \
        -jar ./crypto-worker/build/bin/corda-crypto-worker-5.0.0.0-SNAPSHOT.jar \
	--messagingParams kafka.common.bootstrap.servers=127.0.0.1:9092 \
