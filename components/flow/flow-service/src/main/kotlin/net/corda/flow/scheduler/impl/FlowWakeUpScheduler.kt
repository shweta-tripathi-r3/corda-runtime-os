...
class FlowWakeUpSchedulerImpl constructor(
    ...
    var scheduledWakeUps = Collections.synchronizedMap(mutableMapOf<String, ScheduledFuture<*>>())
    var publisher: Publisher? = null

    override fun onConfigChange(config: Map<String, SmartConfig>) {
        publisher?.close()
        publisher = publisherFactory.createPublisher(PublisherConfig("FlowRPCOps"), config.toMessagingConfig())
    }
