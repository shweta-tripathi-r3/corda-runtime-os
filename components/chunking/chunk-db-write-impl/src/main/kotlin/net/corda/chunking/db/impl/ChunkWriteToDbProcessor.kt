...
internal class ChunkWriteToDbProcessor(
    ...
    override fun onNext(request: Chunk, respFuture: CompletableFuture<ChunkAck>) {
        ...
        publishAckResponseToKafka(request, respFuture, exceptionEnvelope, successful)

        if (!successful) {
            sendFailStatus(request)
        } else {
            validateBinary(request)
        }
    }
    ...
    private fun validateBinary(request: Chunk) {
        val status = validateAllChunks(request.requestId)
        when (status) {
    ...
    private fun validateAllChunks(requestId: String): Status {
        log.debug("Validating chunks for $requestId")

        // Haven't received all the chunks yet?  That's ok, just return
        if (!queries.haveReceivedAllParts(requestId)) return Status.IGNORE
    ...
    private fun persistChunkToDb(chunk: Chunk) {
        queries.persist(chunk)
    }
}
