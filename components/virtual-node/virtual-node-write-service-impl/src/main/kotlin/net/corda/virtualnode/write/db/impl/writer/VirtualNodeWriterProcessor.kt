        ...
        val shortHashes = em.use {
            dbResetRequest.holdingIdentityShortHashes.map { currentVNodeShortHash ->
                ...
                it.transaction { tx ->
                    ...
                    //  I think this could be related to embedding creation of datasources inside entity manager transactions and then
                    //  expecting the entity manager to roll back when liquibase throws. Liquibase handles its own transactionality (one
                    //  transaction per changeset) and so we likely should not imply from the layout of this code that we get
                    //  transactionality, and the code should likely be refactored.
                    //  Example scenario: force upload a CPI with a CPK with a liquibase changelog that attempts to create a table that is
