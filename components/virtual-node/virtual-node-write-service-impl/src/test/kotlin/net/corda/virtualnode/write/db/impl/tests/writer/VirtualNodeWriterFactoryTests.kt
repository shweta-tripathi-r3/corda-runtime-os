...
class VirtualNodeWriterFactoryTests {
    ...
    fun `factory does not start the virtual node writer`() {
        val virtualNodeWriterFactory = VirtualNodeWriterFactory(
            getSubscriptionFactory(), getPublisherFactory(), getDbConnectionManager(), mock(), mock(), mock(), false)
    ...
    fun `factory creates a publisher with the correct configuration`() {
        ...
        val virtualNodeWriterFactory = VirtualNodeWriterFactory(
            getSubscriptionFactory(), publisherFactory, getDbConnectionManager(), mock(), mock(), mock(), false)
    ...
    fun `factory creates an RPC subscription with the correct configuration`() {
        ...
        val virtualNodeWriterFactory = VirtualNodeWriterFactory(
            subscriptionFactory, getPublisherFactory(), getDbConnectionManager(), mock(), mock(), mock(), false)
        virtualNodeWriterFactory.create(expectedConfig)
